...
from .lib.database import get_db # Apne path ke hisaab se check kar lena
from .model import model # Apne path ke hisaab se check kar lena
...
oauth2_scheme=OAuth2PasswordBearer(tokenUrl='auth/login') # y ek security  tool hai jo 
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)):
    ...
    query = select(model.User).where(model.User.email == user_id)
